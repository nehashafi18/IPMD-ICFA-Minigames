...
export default function App() {
  ...
  useEffect(() => {
    async function loadCards() {
      try {
        const data = await apiFetch<CardsResponse>("/api/cards");
  ...
  async function generatePrompt(cards: SelectedCards) {
    ...
    try {
      ...
      formData.append("prompt", promptResult.data.prompt);
      formData.append("negative_prompt", promptResult.data.negative_prompt);

      const result = await apiFetch<GenerateResponse>("/api/art/generate", {
        method: "POST",
        body: formData
      });
      ...
      setGeneratedPrompt(promptResult.data.prompt);
